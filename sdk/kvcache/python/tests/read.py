...
async def test_read():
    ...
    assert matched_key == convert_string_to_list_of_integers("k1"), f"Expected match for key 'k1', but got {matched_key}"
    assert (
        data.get_len() == 768
    ...
    assert (
        matched_key == convert_string_to_list_of_integers("k1")
    ...
    assert matched_key == convert_string_to_list_of_integers("k2"), f"Expected match for key 'k2', but got {matched_key}"
    assert (
        data.get_len() == 768
    ...
    assert matched_key == convert_string_to_list_of_integers("k3"), f"Expected match for key 'k3', but got {matched_key}"
    assert (
        data.get_len() == 768
