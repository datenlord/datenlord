...
impl DatenLordSDK {
    ...
    pub fn new(_py: Python, config_path: String) -> PyResult<Self> {
        ...
        let datenlord_client = pyo3_asyncio::tokio::get_runtime().handle().block_on(async {
            ...
            let node = Node::new(config.node_ip.to_owned(), config.server_port, 1, NodeStatus::Initializing);
            let cluster_manager = Arc::new(ClusterManager::new(kv_engine, node));

            let kvcacheclient = Arc::new(DistributeKVCacheClient::new(cluster_manager, config.storage.block_size));
            tokio::spawn(async move {
                kvcacheclient.start_watch().await;
            });

            Arc::clone(&kvcacheclient)
        });
    ...
    fn py_try_load<'a>(
        ...
        future_into_py(py, async move {
            match datenlord_client.try_load(key).await {
                Ok(value) => Ok(value),
