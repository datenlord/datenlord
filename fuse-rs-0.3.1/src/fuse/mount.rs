...
pub fn umount(short_path: &Path) -> i32 {
    ...
    unsafe { libc::umount2(mntpnt as *const _ as *const u8 as *const i8, 0) }
}
...
pub fn umount(short_path: &Path) -> i32 {
    let mntpnt = short_path.as_os_str();
    unsafe { libc::unmount(mntpnt as *const _ as *const u8 as *const i8, 0) }
}
...
pub fn mount(short_path: &Path) -> i32 {
    ...
    unsafe {
        let result = libc::mount(
            fstype.as_ptr(),
            mntpath.as_ptr(),
            MNT_NOSUID | MNT_NODEV | MNT_NOUSERXATTR,
