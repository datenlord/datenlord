...
/// The default ring load factor, try to update to 75%
const RING_LOAD: u64 = 75;
    ...
    S: BuildHasher,
{
    ...
    pub fn add(&mut self, node: &T, must: bool) -> Option<T> {
        ...
        let thres = match self.capacity.checked_mul(RING_LOAD) {
            Some(thres) => thres.overflowing_div(100).0,
            None => return None,
        };
        while usize_to_u64(self.slots.len()) >= thres && must {
            self.expand();
        }
    ...
    pub fn batch_add(&mut self, nodes: Vec<T>, must: bool) -> Option<Vec<T>> {
        ...
        if must && (usize_to_u64(self.slots.len() + nodes.len()) > self.capacity) {
            if !self.expand() {
                warn!("Expand failed");
...
mod tests {
    ...
    fn test_ring_get_replicas() {
        ...
        let slots = ring.get_replicas(&1_i32, 2).unwrap();
        assert_eq!(slots.len(), 2);
        assert_eq!(slots.get(0).map(|slot| slot.inner().id), Some(1));
        assert_eq!(slots.get(1).map(|slot| slot.inner().id), Some(2));

        let slots = ring.get_replicas(&10_000_i32, 2).unwrap();
        assert_eq!(slots.len(), 2);
        assert_eq!(slots.get(0).map(|slot| slot.inner().id), Some(2));
        assert_eq!(slots.get(1).map(|slot| slot.inner().id), Some(3));
    }
}
