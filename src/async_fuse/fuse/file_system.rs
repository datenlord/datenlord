...
impl<M: MetaData + Send + Sync + 'static> FileSystem for FuseFileSystem<M> {
    ...
    async fn write(
        ...
        let write_result = self.virtual_fs.write(ino, fh, offset, &data, flags).await;
    ...
    async fn flush(
        ...
        let flush_res = self.virtual_fs.flush(ino, fh, lock_owner).await;
    ...
    async fn fsync(
        ...
        let fsync_res = self.virtual_fs.fsync(ino, fh, datasync).await;
