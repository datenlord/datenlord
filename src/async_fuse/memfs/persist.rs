...
impl PersistDirContent {
    /// Get root attr for root dir since there's no parent for root
    ...
    /// New `PersistDirContent` from mem data for persist
...
#[cfg(test)]
#[allow(clippy::match_wild_err_arm, clippy::unwrap_used)]
mod test {
    ...
    async fn test_persist() {
        ...
        for _ in 0_i32..100_i32 {
            ...
            match persist::read_persisted_dir(&mock, path).await {
                Ok(dir) => {
                    assert_eq!(
                        dir.persist_serialized
                            .file_map
                            .iter()
                            .next()
                            .unwrap_or_else(|| panic!())
                            .0,
                        file_map.iter().next().unwrap_or_else(|| panic!()).0
                    );
                }
                Err(_) => {
        ...
        handle.system_end();
        join.await.unwrap_or_else(|_| panic!("join failed"));
    }
}
